...
CFLAGS=-march=rv64gc -mabi=lp64 -msmall-data-limit=0 -fno-pic -fno-pie -Wl,-no-pie -DUSE_RAMDISK=1
...
# Generated userland blobs for embedding into the RAM filesystem
ls: crt0.o ls.c minux.o
	$(CCPREFIX)gcc $(CFLAGS) -O0 -nostdlib -mcmodel=medany -static -nostartfiles -Wl,-e,_start -o ls crt0.o ls.c $(CHILD_CFLAGS)

ls.h: ls
	xxd -i -n ls_elf ls > ls.h

pwd: crt0.o pwd.c minux.o
	$(CCPREFIX)gcc $(CFLAGS) -O0 -nostdlib -mcmodel=medany -static -nostartfiles -Wl,-e,_start -o pwd crt0.o pwd.c $(CHILD_CFLAGS)

pwd.h: pwd
	xxd -i -n pwd_elf pwd > pwd.h

as: crt0.o as.c minux.o
...
as_elf.h: as
	xxd -i -n as_elf as > as_elf.h

ld: crt0.o ld.c minux.o
...
ld_elf.h: ld
	xxd -i -n ld_elf ld > ld_elf.h

nm: crt0.o nm.c minux.o
	$(CCPREFIX)gcc $(CFLAGS) -nostdlib -mcmodel=medany -static -nostartfiles -Wl,-e,_start -o nm crt0.o nm.c $(CHILD_CFLAGS)

nm_elf.h: nm
	xxd -i -n nm_elf nm > nm_elf.h

objdump: crt0.o objdump.c minux.o
	$(CCPREFIX)gcc $(CFLAGS) -nostdlib -mcmodel=medany -static -nostartfiles -Wl,-e,_start -o objdump crt0.o objdump.c $(CHILD_CFLAGS)

objdump_elf.h: objdump
	xxd -i -n objdump_elf objdump > objdump_elf.h

cc_elf.h: cc
	xxd -i -n cc_elf cc > cc_elf.h

sh: crt0.o sh.c minux.o
	$(CCPREFIX)gcc $(CFLAGS) -nostdlib -mcmodel=medany -static -nostartfiles -Wl,-e,_start -o sh crt0.o sh.c $(CHILD_CFLAGS)
	xxd -i sh.elf > sh.h

kernel.elf: minux.o crt0.o cc ls.h pwd.h sh.h as_elf.h ld_elf.h nm_elf.h objdump_elf.h cc_elf.h
...
QEMU_OPTION=-machine virt -bios none -nographic -m 1024M -kernel kernel.elf -nic none
