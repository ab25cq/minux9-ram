...
//#define heap_limit 0xC0000000UL
...
#define PHYSTOP  0xA0000000UL   // 256 MiB span (QEMU is started with -m 1024M)
...
static inline void intr_off_direct(void) {
    // Clear the SIE bit in the sstatus register
    asm volatile("csrc sstatus, %0" : : "r"(SSTATUS_SIE));
}

static inline void intr_on_direct(void) {
    // Set the SIE bit in the sstatus register
